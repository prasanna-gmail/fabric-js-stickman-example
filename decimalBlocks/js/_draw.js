...
function addDrawTools(object) {
	...
	var thicness = object.thickness || 1;
	...
	var drawCanvas = [createCanvas(drawArea[0],drawArea[1],drawArea[2],drawArea[3],true,false,false,zIndex),createCanvas(drawArea[0],drawArea[1],drawArea[2],drawArea[3],true,false,false,zIndex+2)];		
	zIndex += 4;
	var toolsCanvas = createCanvas(drawArea[0],drawArea[1],drawArea[2],drawArea[3],true,false,false,zIndex);
	zIndex++;
	var cursorCanvas = createCanvas(drawArea[0],drawArea[1],drawArea[2],drawArea[3],true,false,true,zIndex);	
	zIndex++;
	cursorCanvas.allowDefault = true;
	...
	draw.cursors = {
		...
		update:function() {
			...
			ctx.translate(35, 15);
				ctx.rotate(-Math.PI/4);
			ctx.translate(-35, -15);
			roundedRect(ctx, 35-25/2, 15-11/2, 25, 11, 4, 2, '#000', '#FEE');
			ctx.strokeStyle = '#000';
			ctx.lineCap = 'round';
			ctx.lineJoin = 'round';
			ctx.lineWidth = 1;
			ctx.beginPath();
			ctx.moveTo(35-25/2+7,15-11/2);
			ctx.lineTo(35-25/2+7,15+11/2);
			ctx.stroke();
			ctx.restore();

			draw.eraserCursorHotspot = [23-r*0.6,27+r*0.6];
			ctx.strokeStyle = '#999';
			ctx.fillStyle = '#FFF';
			ctx.lineWidth = 1;
			ctx.beginPath();
			ctx.arc(draw.eraserCursorHotspot[0],draw.eraserCursorHotspot[1],r,0,2*Math.PI);
			ctx.fill();
			ctx.beginPath();
			ctx.arc(draw.eraserCursorHotspot[0],draw.eraserCursorHotspot[1],r,0,2*Math.PI);
			ctx.stroke();
			draw.eraserCursor = canvas.toDataURL();
			...
			this.eraser = 'url("'+draw.eraserCursor+'") '+draw.eraserCursorHotspot[0]+' '+draw.eraserCursorHotspot[1]+', auto';
	...
	if (typeof object.table == 'object') {
		...
		var tableButton = createCanvas(buttonPos[0],buttonPos[1],40,40,true,false,true,draw.drawButtonZIndex);
		tableButton.type = 'table';
		var dir = object.table.dir || 'below';
		tableButton.menuDir = dir;
		tableButton.draw = function() {
			roundedRect(this.ctx,2,2,36,36,4,4,'#000',draw.buttonColor);
			this.ctx.strokeStyle = '#666';
			this.ctx.lineWidth = 1;
			this.ctx.fillStyle = '#FFF';
			this.ctx.fillRect(10*(40/55),11*(40/55),35*(40/55),33*(40/55));
			this.ctx.fillStyle = '#CCC';
			this.ctx.fillRect(10*(40/55),11*(40/55),35*(40/55),(33/4)*(40/55));
			for (var i = 0; i < 5; i++) {
				this.ctx.moveTo(10*(40/55),(11+i*(33/4))*(40/55));
				this.ctx.lineTo(45*(40/55),(11+i*(33/4))*(40/55));
			}
			for (var i = 0; i < 4; i++) {
				this.ctx.moveTo((10+i*35/3)*(40/55),11*(40/55));
				this.ctx.lineTo((10+i*35/3)*(40/55),44*(40/55));
			}
			this.ctx.stroke();
			this.ctx.lineWidth = 1;
			this.ctx.strokeStyle = '#000';
			this.ctx.strokeRect(10*(40/55),11*(40/55),35*(40/55),33*(40/55));			
		}
		...
		tableSizeCanvas.draw = function() {
			...
			if (draw.tableSizeCanvasDir == 'below') {
				...
				text({context:ctx,fontSize:18,bold:true,text:['Create Table: '+(10-draw.tableSize.rows)+' x '+(draw.tableSize.cols+1)],rect:[12,5,this.data[103],30],align:[-1,-1]});
			}
		}
		...
		addListenerMove(tableSizeCanvas,function(e) {
			updateMouse(e);
			var col = Math.min(draw.tableSize.maxCols,Math.max(0,Math.floor((draw.canvasMouse[0] - e.target.data[100] - 10) / 25))); 
			var row = Math.min(draw.tableSize.maxRows,Math.max(0,Math.floor((draw.canvasMouse[1] - e.target.data[101] - 30) / 25)));		
	...
	if (typeof object.text == 'object') {
		...
		var textButton = createCanvas(buttonPos[0],buttonPos[1],40,40,true,false,true,draw.drawButtonZIndex);
		...
		textButton.draw = function() {
			...
			roundedRect(this.ctx,2,2,36,36,4,4,'#000',color);
			text({context:this.ctx,rect:[0,0,40,40],align:[0,0],fontSize:28,text:['A']});			
		}
	...
	//draw.cursorCanvas.allowDefault = true;
...
function addDrawCanvas() {
	...
	canvas.style.border = draw.drawCanvas.indexOf(canvas) === 0 ? '1px solid black' : '1px solid transparent';
}
...
function changeDrawMode(mode,altMode,taskQuestion,style) {
	if (typeof mode === 'undefined') mode = draw.defaultMode;
	if (typeof altMode === 'undefined') altMode = draw.defaultMode;
	if (mode === draw.drawMode && (un(draw._drawToolsCurrentTaskQuestion) || un(taskQuestion) || draw._drawToolsCurrentTaskQuestion === taskQuestion)) {
		mode = altMode;
	}
	
	//console.log('mode',mode,'style',style);
	
	/*if (!un(draw._saveDrawToolsStyle)) {
		for (var key in draw._saveDrawToolsStyle) {
			draw[key] = draw._saveDrawToolsStyle[key];
		}
		delete draw._saveDrawToolsStyle;
	}
	draw._saveDrawToolsStyle = {
		color:draw.color,
		fillColor:draw.fillColor,
		thickness:draw.thickness,
		dash:draw.dash
	};*/
	if (!un(style)) for (var key in style) draw[key] = style[key];
	
	if (!un(taskQuestion) && mode !== altMode) {
		draw._drawToolsCurrentTaskQuestion = taskQuestion;
		/*if (!un(taskQuestion.drawToolsStyle)) {
			...
			for (var key in taskQuestion.drawToolsStyle) {
				draw[key] = taskQuestion.drawToolsStyle[key];
			}
		}*/
	} else {
		delete draw._drawToolsCurrentTaskQuestion;
	}
	...
	if (['line','rect','square','circle','ellipse','polygon'].includes(mode)) {
		...
		delete draw.currentlyDrawingPaths;
	}
	//console.log('-',mode);
	calcCursorPositions();
	if (typeof toolbar === 'object' && typeof toolbar.draw === 'function') toolbar.draw();
	if (!un(teach) && !un(teach.drawButtons) && typeof teach.drawButtons.update === 'function') teach.drawButtons.update();
	/*if (!un(penButton)) penButton.draw();
	if (!un(penButton2)) penButton2.draw();*/
}

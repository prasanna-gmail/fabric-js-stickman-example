...
textEdit.getClosestTextPos = function(pos,lineNum,matchPos) {
	//if (un(pos)) pos = [mouse.x-draw.drawRelPos[0]-draw.drawArea[0],mouse.y-draw.drawRelPos[1]-draw.drawArea[1]];
...
textEdit.endInput = function(e) {
	...
	if (!un(e)) {
		if (endInputExceptions.indexOf(e.target) > -1 || e.target.textMenu == true) return;
		if (!un(key1[pageIndex]) && key1[pageIndex].indexOf(e.target) > -1) return;
...
textEdit.blinkDraw = function() {
	...
	ctx.clear();
	if (textEdit.blinkState == true && textEdit.selected !== true && !un(textEdit.cursorPos)) {
...
textEdit.paste = function() {
	if (textEdit._type == 'textInput') return;
	if (typeof textEdit.clip == 'string') {
...
textEdit.insert = function(ins,pos,cursorPosDiff) {
	...
	if (typeof ins == "object") {
		...
		var gparent = textObj;
		for (var i = 0; i < pos.length - 2; i++) gparent = gparent[pos[i]];
		...
		if (un(cursorPosDiff)) cursorPosDiff = 1;
		...
		parent[pos[pos.length-2]] = text2.slice(0, slicePos) + ins + text2.slice(slicePos);
		if (un(cursorPosDiff)) cursorPosDiff = ins.length;
...
textEdit.createMenu = function(menu) {
	...
	if (un(menu.colors)) menu.colors = ["#000","#FFF","#F00","#393","#00F","#F0F"]
	if (un(menu.elements)) {
		menu.elements = [
			[
	...
	menu.click = function(e) {
		...
		var x = mouse.x - menu.pos[0];
		var y = mouse.y - menu.pos[1];
	...
	menu.subMenus = [
		...
		{type:"color",offset:-0.5,width:2,height:(menu.colors.length/2),colors:menu.colors,draw:function() {
			var ctx = this.ctx;
			ctx.strokeStyle = "#000";
			ctx.fillStyle = "#CFF";
			ctx.lineWidth = 2;
			...
			for (var c = 0; c < this.colors.length; c++) {
				...
				ctx.fillRect(1+(c%2)*g,1+(Math.floor(c/2))*g,g,g);
				ctx.strokeRect(1+(c%2)*g,1+(Math.floor(c/2))*g,g,g);
				this.buttons.push({type:"color",value:this.colors[c],rect:[1+(c%2)*g,1+(Math.floor(c/2))*g,g,g]});		
			}
		}},
						...
						for (var r = 0; r < obj.cells.length; r++) {
							for (var c = 0; c < obj.cells[r].length; c++) {
								...
								if (type == "align") {
									...
									if (un(cell.box)) cell.box = {type:"none",borderColor:"#000",borderWidth:2,radius:10};

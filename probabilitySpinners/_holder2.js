...
function isEqual(a,b,includeUnderscored) {
	var circularA = [];
	var circularB = [];
	return isEqual2(a,b);
	...
	function isEqual2(a,b) {
		...
		if (a instanceof Array) {
			...
			for (var i = 0, iMax = a.length; i < iMax; i++) {
				if (circularA.indexOf(a[i]) > -1) continue;
				if (circularB.indexOf(b[i]) > -1) continue;
				if (!isEqual2(a[i],b[i])) return false
			}
			for (var i = 0, iMax = b.length; i < iMax; i++) {
				if (circularA.indexOf(a[i]) > -1) continue;
				if (circularB.indexOf(b[i]) > -1) continue;
				if (!isEqual2(b[i],a[i])) return false
			}
			...
			circularB.push(b);
			for (var key in a) {
				if (includeUnderscored !== true && key.indexOf('-') === 0) continue;
				if (!a.hasOwnProperty(key)) continue;
				if (circularA.indexOf(a[key]) > -1) continue;
				if (circularB.indexOf(b[key]) > -1) continue;
			...
			for (var key in b) {
				...
				if (circularA.indexOf(a[key]) > -1) continue;
				if (circularB.indexOf(b[key]) > -1) continue;
				if (!isEqual2(b[key],a[key])) return false
			}
...
var pendingScripts = {
	...
	addScript:function(script) {
		pendingScripts.scripts.push(script);
	},
	scriptLoaded:function(script) {
		if (pendingScripts.scripts.indexOf(script) > -1) pendingScripts.scripts.splice(pendingScripts.scripts.indexOf(script),1);
		if (pendingScripts.scripts.length === 0) {
			for (var i = 0; i < pendingScripts.onLoadFunctions.length; i++) {
				pendingScripts.onLoadFunctions[i]();
			}
			pendingScripts.onLoadFunctions = [];
		}
	},
	onload:function(func) {
		if (pendingScripts.scripts.length === 0) {
			...
			pendingScripts.onLoadFunctions.push(func);
		}
	}
};
...
//funcLog();
/*function funcLogStart() {
	window.functionLog = [];
	augment(function(name, fn) {
		window.functionLog.push(name);
	});
}
function funcLogStop() {
	console.log(window.functionLog);
}*/
...
function updateMouse(e) {
	...
	if (!un(draw) && !un(draw.drawCanvas) && !un(draw.drawCanvas[0])) {
		...
		draw.canvasMouse = [(x - canvasBounds.left) * (mainCanvasWidth / canvasBounds.width), (y - canvasBounds.top) * (mainCanvasHeight / canvasBounds.height)];
				
		if (!un(draw.div)) {
			var bounds = draw.div.getBoundingClientRect();
			var x2 = (x - bounds.left) * (1225 / bounds.width);
			var y2 = (y - bounds.top) * (700 / bounds.height);
			draw.div.mouse = [x2,y2];
		}
	}
}
...
window.addEventListener("touchstart", pointerEventPreventDefault, true);
window.addEventListener("touchmove", pointerEventPreventDefault, true);
window.addEventListener("touchend", pointerEventPreventDefault, true);
window.addEventListener("pointerstart", pointerEventPreventDefault, true);
window.addEventListener("pointermove", pointerEventPreventDefault, true);
window.addEventListener("pointerend", pointerEventPreventDefault, true);
window.addEventListener("mousedown", pointerEventPreventDefault, true);
window.addEventListener("mousemove", pointerEventPreventDefault, true);
window.addEventListener("mouseup", pointerEventPreventDefault, true);
function pointerEventPreventDefault(e) {
	if (e.target.allowDefault !== true) {
		e.preventDefault();
		return false;
	}
	if (typeof e.touches === 'object' && e.touches.length > 1) {
		e.preventDefault();
		return false;
	}
	//console.log(e.type, e.target, e.target.nodeName, e.target.allowDefault === true);
}
/*
...
function loadScripts1() {
	for (var i = 0; i < scriptsToLoad.length; i++) {
		loadScript(scriptsToLoad[i], scriptLoaded, scriptLoaded);
	}
}
function loadScript(url, callback, errorCallback, index) {
	...
	script.src = url2;
	script.index = index;
	pendingScripts.addScript(script);
	script.onload = function() {
		pendingScripts.scriptLoaded(this);
		//if (pendingScripts.indexOf(this) > -1) pendingScripts.splice(pendingScripts.indexOf(this),1);
		if (!un(callback)) callback(this.index);
	};
	script.onerror = function() {
		pendingScripts.scriptLoaded(this);
		//if (pendingScripts.indexOf(this) > -1) pendingScripts.splice(pendingScripts.indexOf(this),1);
		console.log('script not loaded',url);
...
function newctx(obj) {
	...
	if (typeof cnv.ctx.setLineDash !== 'function') cnv.ctx.setLineDash = function () {};

...
function drawGrid3(context,contextLeft,contextTop,gridDetails,opt_fontSize,opt_minorColor,opt_majorColor,opt_xAxisColor,opt_yAxisColor,opt_borderColor,opt_originColor,opt_backColor,opt_showGrid,opt_showScales,opt_showLabels) {
	...
	********************************/
	...
	if (showAxes == true) {
		...
		//if (y0 >= top-0.01 && y0 <= top+height+0.01) {
			context.moveTo(left, y0DisplayPos);
			context.lineTo(left + width-10*sf, y0DisplayPos);	
		//}
		context.closePath();
		context.stroke();
		context.beginPath();
		...
		//if (x0 >= left-0.01 && x0 <= left+width+0.01) {
			context.moveTo(x0DisplayPos, top+10*sf);
			context.lineTo(x0DisplayPos, top + height);	
		//}
		context.closePath();
		context.stroke();
...
function getGridCoordAtDrawPos(grid,pos,roundTo) {
	var coords = [
		grid.xMin + (grid.xMax - grid.xMin) * (pos[0] - grid.left) / grid.width,
		grid.yMax - (grid.yMax - grid.yMin) * (pos[1] - grid.top) / grid.height
	];
	if (typeof roundTo === 'number' && roundTo > 0) coords = [roundToNearest(coords[0],roundTo),roundToNearest(coords[1],roundTo)];
	return coords;
}
